...
public class FlutterLocalNotificationsPlugin implements MethodCallHandler, PluginRegistry.NewIntentListener, FlutterPlugin, ActivityAware {
    ...
    private static final String GET_INTENT = "get_intent";
    private static final String REMOVE_INTENT = "remove_intent";
    ...
    public void onMethodCall(MethodCall call, Result result) {
        switch (call.method) {
            ...
            case GET_INTENT: {
                getIntentPayload(call, result);
                break;
            }

            case REMOVE_INTENT: {
                removeIntent(call, result);
                break;
            }
    ...
    private Boolean getIntentPayload(MethodCall call, Result result) {
        if(mainActivity.getIntent().getStringExtra(PAYLOAD)!=null){
            String payload = mainActivity.getIntent().getStringExtra(PAYLOAD);
            result.success(payload);
            mainActivity.getIntent().removeExtra(PAYLOAD);
            return true;
        }else{
            result.success(null);
            return false;
    ...
    private Boolean removeIntent(MethodCall call, Result result) {
        if(mainActivity.getIntent().getStringExtra(PAYLOAD)!=null){
            mainActivity.getIntent().removeExtra(PAYLOAD);
            result.success(true);
            return true;
